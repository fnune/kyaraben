...
func (m *Manager) EnvironmentConfigPath() (string, error) {
	configDir, err := paths.ConfigDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(configDir, "environment.d", "kyaraben.conf"), nil
}

const environmentTemplate = `# Generated by kyaraben - sets up PATH and XDG_DATA_DIRS
# This file is read by systemd user sessions to discover kyaraben binaries and .desktop files
# Changes here will be overwritten by kyaraben apply
PATH=${PATH}:$HOME/.local/state/kyaraben/bin
XDG_DATA_DIRS=${XDG_DATA_DIRS:-/usr/local/share:/usr/share}:$HOME/.local/state/kyaraben/share
`

func (m *Manager) GenerateEnvironmentConfig() error {
	envPath, err := m.EnvironmentConfigPath()
	if err != nil {
		return fmt.Errorf("getting environment config path: %w", err)
	}

	if err := os.MkdirAll(filepath.Dir(envPath), 0755); err != nil {
		return fmt.Errorf("creating environment.d directory: %w", err)
	}

	if err := os.WriteFile(envPath, []byte(environmentTemplate), 0644); err != nil {
		return fmt.Errorf("writing environment config: %w", err)
	}

	log.Info("Generated environment config: %s", envPath)
	return nil
}

func (m *Manager) RemoveEnvironmentConfig() error {
	envPath, err := m.EnvironmentConfigPath()
	if err != nil {
		return err
	}
