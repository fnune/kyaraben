...
func (c *Config) Generate(ctx model.GenerateContext) (model.GenerateResult, error) {
	...
	downloads, err := retroarch.CoreShaderDownloads(model.EmulatorIDRetroArchBsnes, ctx.BaseDirResolver, pc)
	if err != nil {
		return model.GenerateResult{}, err
	}
	systems := []model.SystemID{model.SystemIDSNES}
	embeddedFiles, err := retroarch.CoreEmbeddedFiles(model.EmulatorIDRetroArchBsnes, systems, pc, ctx.BaseDirResolver)
	if err != nil {
		return model.GenerateResult{}, err
	}

	patches := retroarch.CorePatches(model.EmulatorIDRetroArchBsnes, ctx.Store, ctx.ControllerConfig, pc, ctx.BaseDirResolver)
	patches = append(patches, retroarch.OverlayPatches(model.EmulatorIDRetroArchBsnes, systems, pc, ctx.BaseDirResolver)...)
