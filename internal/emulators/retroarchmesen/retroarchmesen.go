...
func (c *Config) Generate(ctx model.GenerateContext) (model.GenerateResult, error) {
	...
	downloads, err := retroarch.CoreShaderDownloads(model.EmulatorIDRetroArchMesen, ctx.BaseDirResolver, pc)
	if err != nil {
		return model.GenerateResult{}, err
	}
	systems := []model.SystemID{model.SystemIDNES}
	embeddedFiles, err := retroarch.CoreEmbeddedFiles(model.EmulatorIDRetroArchMesen, systems, pc, ctx.BaseDirResolver)
	if err != nil {
		return model.GenerateResult{}, err
	}

	patches := retroarch.CorePatches(model.EmulatorIDRetroArchMesen, ctx.Store, ctx.ControllerConfig, pc, ctx.BaseDirResolver)
