...
func (c *Config) Generate(ctx model.GenerateContext) (model.GenerateResult, error) {
	...
	downloads, err := retroarch.CoreShaderDownloads(model.EmulatorIDRetroArchGenesisPlusGX, ctx.BaseDirResolver, sc)
	if err != nil {
		return model.GenerateResult{}, err
	}
	return model.GenerateResult{
		Patches:          retroarch.CorePatches(model.EmulatorIDRetroArchGenesisPlusGX, ctx.Store, ctx.ControllerConfig, sc, ctx.BaseDirResolver),
		Symlinks:         symlinks,
