...
func (c *Config) Generate(ctx model.GenerateContext) (model.GenerateResult, error) {
	...
	downloads, err := retroarch.CoreShaderDownloads(model.EmulatorIDRetroArchMelonDS, ctx.BaseDirResolver, pc)
	if err != nil {
		return model.GenerateResult{}, err
	}
	return model.GenerateResult{
		Patches:          retroarch.CorePatches(model.EmulatorIDRetroArchMelonDS, ctx.Store, ctx.ControllerConfig, pc, ctx.BaseDirResolver),
		Symlinks:         symlinks,
