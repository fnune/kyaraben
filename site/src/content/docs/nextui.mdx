...
- Keep deleted ROMs: whether to keep this device's ROMs when they are deleted elsewhere (disabled by default)
...
With `ignore_delete_roms = true`, deleting a ROM on another device no longer removes it from this one, the same protection Kyaraben devices get. It is off by default here because handheld cards fill up quickly and following deletions is usually how you reclaim that space.

Turn it on when the handheld is the device you want to stay complete. The setting is per device, so it protects this device only, and a rename on another device arrives as a delete plus a create, which can leave the old file behind as a duplicate.
...
- Saves and states keep 30 days of [file versions](/collection/#file-versioning) on the card, the same as Kyaraben devices, so a deletion can be undone
- [ROM deletion protection](/collection/#protecting-roms-from-deletion) is off by default here, the opposite of Kyaraben devices. A handheld is usually short on space, so it follows ROM deletions to free the card. Set `ignore_delete_roms = true` under `[service]` to keep this device's ROMs instead, and keep the device you treat as your library protected either way.
