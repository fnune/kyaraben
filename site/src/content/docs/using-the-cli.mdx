...
| Key      | Type   | Default    | Description                                                       |
| -------- | ------ | ---------- | ----------------------------------------------------------------- |
| `resume` | string | `autosave` | Global resume setting: `autosave`, `autoload`, `off`, or `manual` |

`autosave` writes a savestate when you quit. `autoload` does that and loads the
savestate back on the next launch. Both apply only to emulators that support
resume reliably, and per-emulator overrides in `[emulators.<id>]` take
precedence.

The older value `recommended` is still accepted and now means `autosave`:
autoload is opt-in, because reloading the last savestate on every launch makes
it easy to overwrite progress you meant to keep.
...
- `resume`: Auto-resume behaviour. Values: `autosave`, `autoload`, `off`,
  `manual`. When omitted, follows the global `[savestate]` setting. The older
  value `on` is still accepted and means `autosave`.
...
With `--headless`, `kyaraben apply` creates directories for all systems but does not install emulators. See [headless server guide](/guides/headless-server/) for setup instructions.
