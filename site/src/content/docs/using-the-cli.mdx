...
Configuring emulators...
  Built: ~/.local/state/kyaraben/build/nix/.nix-portable/nix/store/...
...
Checking provisions...
  ...
  DuckStation (psx)
    ✗ scph5501.bin - MISSING (required)
    - scph5500.bin - not found (optional)
    - scph5502.bin - not found (optional)
  ...
  melonDS (nds)
    - bios7.bin - not found (optional)
    - bios9.bin - not found (optional)
    - firmware.bin - not found (optional)
...
This will remove:
  ...
  ~/.local/state/kyaraben (nix store, manifest, state)
...
Enable sync in your config.toml:
