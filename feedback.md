...
## Document how emulator CLI commands are generated

It's unclear how Kyaraben generates the correct CLI command for each emulator (or RetroArch + core). This includes:
- Where the Exec line for desktop files is constructed
- How RetroArch cores get the `-L <core>` argument
- How the `%ROM%` placeholder in ES-DE's es_systems.xml maps to actual ROM paths

The relevant code appears to be in:
- `internal/frontends/esde/config.go` - `buildCommand()` generates ES-DE's command strings
- `internal/apply/apply.go` - generates desktop file Exec lines
- `model.LauncherInfo` - stores binary and args

This should be documented in MODEL.md or a separate doc for maintainability.

The current implementation in `buildCommand()` assumes all emulators take a positional ROM argument (`<binary> %ROM%`), but this is likely wrong. Different emulators have different CLI conventions:
- Some use positional args: `duckstation game.iso`
- Some use flags: `emulator --rom game.iso` or `emulator -r game.iso`
- RetroArch needs `-L <core>` before the ROM
- Some may need additional flags for fullscreen, config paths, etc.

The ROM argument pattern should probably be part of the emulator definition (in `LauncherInfo` or similar), not hardcoded in the frontend's command builder.
