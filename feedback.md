...
- On Steam Deck, clicking on 'Open Syncthing web interface' opens Discover
    ...
    - Might be a SteamOS xdg-open issue
...
- RetroArch assets missing after reinstall (requires reinstall to trigger extraction)
...
- Config ownership model: OwnedFile/OwnedRegion to let emulators declare exactly which sections of config files they own, enabling separate profile files (DuckStation, PCSX2, Dolphin, Eden, Flycast) without stomping unrelated settings
- Keyboard bindings generation: generate keyboard hotkey bindings alongside controller bindings for emulators that support it
- Desktop controller model: user-selectable controller model for non-Steam setups where GUID detection does not map to Steam Deck Neptune controller
